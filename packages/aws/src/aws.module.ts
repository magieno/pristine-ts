  ...
  for (const dynamicTableName of dynamicTableNameRegistry) {
    ...
    if (container.isRegistered(dynamicTableName.tokenName) === false) {
      ...
      try {
        ...
        logHandler.debug("AwsModule: Successfully registered table name.", {
          extra: {
            tokenName: dynamicTableName.tokenName,
            value
          }
        })
      } catch (e) {
    ...
    try {
      ...
      const logHandler: LogHandlerInterface = container.resolve("LogHandlerInterface");
      logHandler.error("AwsModule: Error resolving the dynamic table token name.", {
        extra: {
          error,
          tokenName: dynamicTableName.tokenName
        }
