...
describe("Event Bridge event parser", () => {
  ...
  it("should support an event from Event Bridge", () => {
    ...
    expect(eventBridgeEventParser.supportsMapping(rawEvent, {
  ...
  it("should support a scheduled event from Event Bridge", () => {
    ...
    expect(eventBridgeEventParser.supportsMapping(scheduledRawEvent, {
      keyname: ExecutionContextKeynameEnum.AwsLambda,
      context: {}
    })).toBeTruthy();
  });
  ...
  it("should transform a normal event bridged from Event Bridge", () => {
    ...
    expect(eventBridgeEventParser.map(rawEvent, {
      keyname: ExecutionContextKeynameEnum.AwsLambda,
      context: {}
    })).toEqual({
  ...
  it("should transform a scheduled event bridged from Event Bridge", () => {
    ...
    expect(eventBridgeEventParser.map(scheduledRawEvent, {
      keyname: ExecutionContextKeynameEnum.AwsLambda,
      context: {}
    })).toEqual({
      events: [eventBridgePayloadEvent],
