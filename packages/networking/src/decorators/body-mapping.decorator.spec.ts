...
describe("Body Mapping Decorator", () => {
  it("should properly set the context for a ClassType", () => {
    ...
    const context: {
      [bodyMappingDecoratorMetadataKeyname]: ClassTransformerBodyMappingContextInterface
  ...
  it("should properly set the context for a function", () => {
    const method: ((body: any, dataMapper: DataMapper) => any) = (body) => {
      return {...body, "a": true}
    };
    ...
    const context: {
      [bodyMappingDecoratorMetadataKeyname]: FunctionBodyMappingContextInterface
    } = MetadataUtil.getRouteContext(Test2.prototype, "route");
  ...
  it("should properly set the context for a function", () => {
    ...
    const context: {
      [bodyMappingDecoratorMetadataKeyname]: DataMappingBuilderBodyMappingContextInterface
    } = MetadataUtil.getRouteContext(Test3.prototype, "route");
