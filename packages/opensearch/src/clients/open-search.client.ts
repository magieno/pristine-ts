...
export class OpenSearchClient {
  ...
  async search<T>(indexName: string, query: Query): Promise<SearchResult<T>> {
    try {
      ...
      await this.logHandler.debug("OpenSearchClient: Received response from OpenSearch.", {
        extra: {
          response,
          indexName,
          query
      ...
      await this.logHandler.error("OpenSearchClient: Error while searching for documents.", {
        extra: {
          error: e,
          indexName,
          query
