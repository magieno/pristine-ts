...
export class OpenSearchClient {
    ...
    async search<T>(indexName: string, resultClassObject: T, query: Query): Promise <SearchResult<T>> {
            try {
                ...
                await this.logHandler.debug("ELASTICSEARCH CLIENT - Searching for query.", {elasticSearchQuery: query}, ElasticSearchModuleKeyname);
                ...
                await this.logHandler.debug("ELASTIC SEARCH CLIENT - Querying elasticsearch with params.", {params}, ElasticSearchModuleKeyname);

                const response = await client.search(params);

                await this.logHandler.debug("ELASTIC SEARCH CLIENT - Received response", {response, indexName, resultClassObject, elasticSearchQuery: query}, ElasticSearchModuleKeyname);

                return await this.parseResponse(response, resultClassObject, query.page, query.maximumNumberOfResultsPerPage);
            } catch (e) {
                await this.logHandler.error("ELASTIC SEARCH CLIENT", {error: e, indexName, resultClassObject, elasticSearchQuery: query}, ElasticSearchModuleKeyname);
